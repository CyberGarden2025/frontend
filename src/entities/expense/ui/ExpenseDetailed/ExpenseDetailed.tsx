...
import { ExpenseUpdateCategory } from '../ExpenseUpdateCategory';
...
export const ExpenseDetailed: FC<ExpenseDetailedProps> = ({ transaction }) => {
    ...
    return (
        <div className={cls.wrapper}>
            <ExpenseUpdateCategory id={transaction.id} isOpen={isOpen} setIsOpen={setIsOpen} />
            <div className={cls.card}>
                <p className={cls.text}>
                    {transaction.sum > 0 ? 'Пополнение на счет' : 'Списание с счета'}
                </p>
                <div className={cls.cardIcon}>
                    <CardIcon />
                    <p className={cls.cardIconText}>*0920</p>
                </div>
            </div>
            ...
            <div className={clsx(cls.card, cls.sumCard)}>
                <div className={cls.row}>
                    <div className={cls.textCol}>
                        <h3 className={cls.subTitle}>Неизвестно</h3>
                        <p className={cls.category}>{readableCategory[transaction.category]}</p>
                    </div>
                    <div className={cls.iconCol}>
                        <div className={cls.icon}>{categoryIconMapping[transaction.category]}</div>
                        <h2
                            className={clsx(cls.sum, {
                ...
                <Button
                    label={
                        <div className={cls.buttonContent}>
                            <p className={clsx(cls.actionText, cls.warning)}>Удалить</p>
                        </div>
                    }
                />
                <Button
                    ...
                    label={
                        <div className={cls.buttonContent}>
                            <EditIcon />
                            <p className={cls.actionText}>Категория</p>
                        </div>
                    }
                />
            </div>
            ...
            <div className={clsx(cls.card, cls.scan)}>
                <div className={cls.col}>
                    ...
                    <p className={cls.text}>сохраните информацию о покупке</p>
                    <div
                        className={cls.icon}
                    ...
                    <ul className={cls.list}>
                        {transaction.purchases.map(item => (
                            <li className={cls.item}>
                                ...
                                <p className={cls.count}>{item.count}кг</p>
            ...
            <div className={clsx(cls.card, cls.details)}>
                ...
                <div className={cls.detailsWrapper}>
                    <p className={cls.text}>Идентификатор операции</p>
                    <h5 className={cls.subTitle}>{transaction.refNo}</h5>
                </div>
            </div>
        </div>
    );
